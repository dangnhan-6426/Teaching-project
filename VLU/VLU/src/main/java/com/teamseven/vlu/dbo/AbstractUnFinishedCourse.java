...
public abstract class AbstractUnFinishedCourse<T> {
    ...
    protected abstract List<T> getUnFinishedCourse(Connection connection, Object id) throws SQLException;
    ...
    public List<T> getUnFinishedCourse(Object id) throws SQLException {

        List<T> list = new ArrayList<>();

        try {
            connection = DBUtils.makeConnection();
            list = getUnFinishedCourse(connection, id);
        } finally {
            connection.close();
        }
        return list;
