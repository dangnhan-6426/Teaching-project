...
public class UnFinishedCourseFacade extends AbstractUnFinishedCourse<UnFinishedCourse> {
    ...
    private static final String SQL_GET_UNFINISHED_COURSE = "SELECT * FROM UnFinishedCourse WHERE LearnerID = ?";
    ...
    @Override
    protected List<UnFinishedCourse> getUnFinishedCourse(Connection connection, Object id) throws SQLException {
        ArrayList<UnFinishedCourse> unFinishedCourseAllList = new ArrayList<>();

        try {
            if (connection != null) {
                preparedStatement = connection.prepareStatement(SQL_GET_UNFINISHED_COURSE);
                preparedStatement.setString(1, id.toString());
                resultSet = preparedStatement.executeQuery();
                while (resultSet.next()) {
                    UnFinishedCourse unFinishedCourse = getInfoUnFinishedCourseFromSQL(resultSet);
